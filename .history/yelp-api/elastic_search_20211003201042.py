...
def get_table(table_name):
    ...
    while True:
        ...
        deserialized_document = {k: deserializer.deserialize(v) for k, v in response.items()}

        results.extend(deserialized_document['Items'])
        break
...
index_doc = []
for row in data:
    deserialized = deserializer.deserialize(row)
    index_doc.append({
        'business_id': deserialized['business_id'],
        'categories': deserialized['categories']
    })
print(index_doc)
dump_table(index_doc)