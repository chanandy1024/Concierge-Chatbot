...
def validate(city, cuisine_type, date, phoneNumber, time):
    if city and not isvalid_city(city):
        return build_validation_result(False, 'City', 'We currently do not support {} as a valid destination. Can you try a different area? Currently, Manhattan is the only supported area'.format(city))
    if date:
        if not isvalid_date(date):
            return build_validation_result(False, 'Date', 'I did not understand your arrival date.  When would you like to dine in?')
        if datetime.datetime.strptime(date, '%Y-%m-%d').date() + datetime.timedelta(hours= 3) < datetime.date.today():
            return build_validation_result(False, 'Date', 'Reservations must be scheduled today or after. Can you try a different date?')
    if time:
        if not isvalid_date(time):
            return build_validation_result(False, 'Time', 'I did not understand your arrival time.  What time would you like to dine in?')

        d = datetime.datetime.strptime(date, '%Y-%m-%d') + datetime.timedelta(hours= 3)
        t = datetime.datetime.strptime(time, '%H:%M').time()
        if datetime.datetime.combine(d, t) < datetime.datetime.now():
            return build_validation_result(False, 'Time', 'Reservations must made at a time later than the current one. Can you try a different time?')
...
def DiningSuggestions(intent_request):
    ...
    if intent_request['invocationSource'] == 'DialogCodeHook':
        ...
        validation_result = validate(city, cuisine, date, phoneNumber, time)
    ...
    response = sqs_client.send_message(
        QueueUrl=os.environ['AWS_OS_HOST'],
